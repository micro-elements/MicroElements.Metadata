...
namespace MicroElements.Metadata.Serialization
{
    ...
    public static class PropertyContainerMapper
    {
        ...
        /// <returns>Result property container.</returns>
        public static PropertyContainer<TSchema> ToPropertyContainer<TSchema>(this object sourceData, bool includePropertiesNotFromSchema = false)
            where TSchema : IPropertySet, new()
        {
            var propertyComparer = PropertyComparer.ByTypeAndNameIgnoreCaseComparer;
            ...
            IPropertyValueFactory propertyValueFactory = new CachedPropertyValueFactory(new PropertyValueFactory(), propertyComparer);
